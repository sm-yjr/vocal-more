...
2. 即使是 minimal，也要删除不承载信息的语气词、停顿词、思考填充、口吃重复和无意义废话，如"嗯""啊""呃""那个""就是""就是说""然后那个""这种啊""那种""这样那样"等
3. 删除不影响含义的不必要连词、垫词和绕口铺垫；只保留真正承担逻辑关系的连接词
...
输出：我想说一下，我们的 API 响应时间最近变慢了，然后用户那边有投诉。""",
    "balanced": """示例：
输入：嗯那个我想说一下就是我们的 API 响应时间最近变慢了然后用户那边有投诉
...
LEVEL_INSTRUCTIONS = {
    "minimal": "在满足上述口语转文本基线的前提下，尽量保留原句、原词和口语感。默认删除无信息的语气词、停顿词、思考填充、明显口吃重复、无意义连词垫词，以及明确被后文推翻的自我修正过程。除这些口语噪音外，不要主动书面化，不要主动压缩有效信息，不要明显改写句式。",
