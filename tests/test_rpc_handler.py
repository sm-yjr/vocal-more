...
def test_dispatch_set_mode(handler):
    ...
    from vocal_more.rpc_handler import RPCError
    with pytest.raises(RPCError, match="Unknown mode"):
        handler.dispatch("set_mode", {"mode": "meeting"})
    assert handler.dispatch("initialize", {})["current_mode"] == "realtime_long"
...
def test_retired_meeting_endpoint_is_unavailable(handler):
    from vocal_more.rpc_handler import RPCError
    with pytest.raises(RPCError, match="Method not found"):
        handler.dispatch("generate_meeting_notes", {"id": "existing-recording"})
    assert not hasattr(handler, "_meeting_tasks")
