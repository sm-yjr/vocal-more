...
菜单栏“看屏幕说话”是显式、单次启用的隐私边界。macOS 主显示器按 720p 上限压缩成内存 JPEG，原始大小不超过 190 KiB；首个音频块发出后通过 `input_image_buffer.append` 发送，录音期间每 2 秒更新一帧，230 秒后停止视频帧以留出官方 240 秒视频上限。会话设置 `representation_compact=normal`，屏幕内容只帮助识别界面文本、代码和专有名词，不允许模型描述屏幕；帧不写入录音历史或诊断日志。
