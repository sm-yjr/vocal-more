...
“看屏幕说话”的 CoreGraphics 截帧在薄 UI 的既有系统操作通道执行，AppKit 主线程只处理触发和结果事件。JPEG 帧使用独立、容量为 2 的视觉队列进入 Rust 会话；队列满时丢弃当前视觉更新，不占用 PCM 输入队列，也不会阻塞或降级音频采集。会话代次在 RPC 和 Core 两层校验，结束后到达的晚帧直接丢弃。屏幕帧不进入录音写入器、历史仓储或诊断输出。
